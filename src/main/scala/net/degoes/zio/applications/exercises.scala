...
object hangman extends App {
  ...
  def gameLoop(state0: State): ZIO[Console, IOException, Unit] =
    for {
      _      <- putStrLn("Please enter a guess")
      char   <- getChoice
      state  = state0.addChar(char)
      result = guessResult(state0, state, char)
      _      <- (renderState(state))
      loop <- result match {
               case GuessResult.Won       => putStrLn(s"well done ${state.name}") as false
               case GuessResult.Lost      => putStrLn(s"${state.name} you got hanged") as false
               case GuessResult.Correct   => putStrLn("") as true
               case GuessResult.Incorrect => putStrLn("bad guess") as true
               case GuessResult.Unchanged => putStrLn("You already guessed that one") as true
             }
  ...
  lazy val getChoice: ZIO[Console, IOException, Char] = {
    getStrLn.map(_.trim.toLowerCase.toList).flatMap {
      ...
      case _                                   => putStrLn("You didn't enter any characters") *> getChoice
    }
  }
  ...
  lazy val chooseWord: ZIO[Random, Nothing, String] = {
    (for {
      index <- zio.random.nextInt(Dictionary.length)
